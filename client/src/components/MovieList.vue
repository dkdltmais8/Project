...
<div class='black'>
  ...
  id="Show-btn" 
  @click.native="$bvModal.show('modal1'), chooseId=movie.id"
  ...
  <b-modal id="modal1" centered size="lg" hide-header-close>
...
export default {
  ...
  methods : {
    ...
    getMovie () {
      axios ({
        method:'get',
        url: 'https://api.themoviedb.org/3/movie/popular?api_key=33b865b38e340385996befbfd40fadd0&language=ko-KR&page=1',
      })
      .then((response)=> {
        this.movies = response.data.results.map(({title, poster_path, popularity, id}) => ({
          title, popularity, 
